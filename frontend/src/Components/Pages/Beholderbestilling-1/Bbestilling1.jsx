...
const Bbestilling1 = () => {
  ...
  // state that store the selectedValue (selectedDat)
  const dispatch = useDispatch(); // the way we send data to redux store
  console.log("Selected Data:", selectedData);

  const handleRadioChange = (dataId) => {
    const selectedData = events.find((data) => data.id === dataId);
    dispatch(setSelectedValue(selectedData)); // Dispatch the action to update the selected value (selectedId is the value od the id)
  };
